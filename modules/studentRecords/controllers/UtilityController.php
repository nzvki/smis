...
use app\models\NduAdmittedStudent;
...
//                Yii::$app->session->setFlash('error', 'Error Uploading File');
     ...
     * @return string|Response
     */
    public function actionNduUploadAdmittedList(): Response|string
    {
        $model = new NduAdmittedStudent();
...
//                Yii::$app->session->setFlash('error', 'Error Uploading File');
            ...
            Yii::$app->session->set('time_taken',time() - $time);
            return $this->redirect(['ndu-upload-admitted-list']);

        }

        return $this->render('ndu-upload-admitted-list',['model'=>$model]);
    }
...
//            Yii::$app->session->setFlash('error', 'The requested resource does not exist.');
     ...
     * @return void|\yii\console\Response|Response
     */
    public function actionNduTemplateDownload(string $file='ndu_admitted_student_template', string $fpath='')
    {
        $path = Yii::getAlias('@app') . '/templates'.'/'.$file.'.xlsx';
        if(strtoupper($file)==='EFILE'){
            $path=$fpath;
...
//            Yii::$app->session->setFlash('error', 'The requested resource does not exist.');
     ...
     * @return void
     */
    public function processSheetData($fileName): void
    {
        ini_set('max_execution_time', 60*10); // 10 mins
        ini_set('memory_limit', '2048M'); // 2GB
        try {
            ...
            $sheet = $objExcel->getActiveSheet();
            $head = ['PROGRAMME CODE','SURNAME','OTHER NAMES',
                'GENDER','NATIONALITY','DATE OF BIRTH','NATIONAL ID NO','PASSPORT NO','SERVICE NO','SERVICE'];
            $eSheet->fromArray([$head],NULL,'A1');
            $row = $eSheet->getHighestRow()+1;
            $colCount = 1 + (ord($sheet->getHighestColumn()) - ord('A'));
            ...
            for ($i = 1; $i <= $rowCount; $i++) {
                ...
                $programme_code = trim($sheet->getCell("A$i"));
                $surname = trim(strtoupper($sheet->getCell("B$i")));
                $otherName = trim(strtolower($sheet->getCell("C$i")));
                $gender = trim(strtolower($sheet->getCell("D$i")));
                $nationality = trim(strtolower($sheet->getCell("E$i")));
                $date_of_birth = trim($sheet->getCell("F$i"));
                $national_id_no = trim(strtolower($sheet->getCell("G$i")));
                $passport_no = trim(strtolower($sheet->getCell("H$i")));
                $service_no = trim(strtolower($sheet->getCell("I$i")));
                $service = trim(strtolower($sheet->getCell("J$i")));
                $model = NduAdmittedStudent::find()
                    ->where(['service_number' => $service_no])
                    ->one();
                if($model!==null){
                    $error_count++;
                    $eSheet->fromArray([$programme_code,$surname,$otherName,
                        $gender,$nationality,
                        $date_of_birth,$national_id_no,$passport_no,$service_no,
                        $service,'Already Exists'],NULL,"A$row");
                    $eSheet->getStyle("A$row:M$row")->getFill()->setFillType('solid')
                        ->getStartColor()->setRGB('FFA500');
                    $row++;
                }else {
                    ...
                    // Set the DB attributes
                    $model->uon_prog_code = $programme_code;
                    $model->surname = strtoupper($surname);
                    $model->other_names = strtoupper($otherName);
                    $model->gender = strtoupper($gender);
                    $model->nationality = $nationality;
                    $model->date_of_birth = Yii::$app->formatter->asDate($date_of_birth, 'php:Y-m-d');
                    $model->national_id = $national_id_no;
                    $model->passport_no = $passport_no;
                    $model->service_number = $service_no;
